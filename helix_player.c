...
// Called before starting decoding of each frame.
{
	...
	// TODO: that shoud be better a circular buffer!!!
 ...
 * Returns number of frames decoded, which is 0 if the format is wrong. */
{
	...
	if (file_open(FOPEN_MODE_R) == FALSE) {
		LED_Toggle(LED2);
		print_dbg(CL_RED"OpenError\n");
		return 0;
	}
	segment.u16_size = 0;
	segment.u32_addr = 0;
	...
	while(1) {
		...
		if (err || playback_abort) {
			print_dbg(CL_RED"ERR: ");
			print_dbg_char_hex(-err);
			print_dbg("\n");
			break;
		}
